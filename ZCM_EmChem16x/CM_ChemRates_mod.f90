...
module ChemRates_mod
  ...
  implicit none
  private
  character(len=*),parameter, public :: CM_schemes_ChemRates = " EmChem16 ShipNOx Aero2017nx VBS_acp2012 FFireInert SeaSalt Dust"
...
contains
  ...
  subroutine setChemRates()
    ...
    rct(72,:) = (((7.4e-31*M*(TEMP/300)**(-2.4))*(3.3e-11*(TEMP/300)  &
              ...
              & **(-0.3))))
    rct(73,:) = UPTAKERATE(CNO3,  &
              & GAM=0.001,  &
              & S=S_M2M3(AERO%PM,  &
              & :))
    rct(74,:) = UPTAKERATE(CHNO3,  &
              & GAM=0.1,  &
              & S=S_M2M3(AERO%DU_C,  &
              & :))
    rct(75,:) = UPTAKERATE(CHNO3,  &
              & GAM=0.01,  &
              & S=S_M2M3(AERO%SS_C,  &
              & :))
    rct(76,:) = UPTAKERATE(CHO2,  &
              & GAM=0.2,  &
              & S=S_M2M3(AERO%PM,  &
              & :))
    rct(77,:) = 6.3e-16*EXP(-580.0*TINV)
    rct(78,:) = 1.2e-11*EXP(444.0*TINV)
    rct(79,:) = 1.2e-12*EXP(490.0*TINV)
    rct(80,:) = 0.914*(2.91e-13*EXP(1300.0*TINV))
    rct(81,:) = 4.0e-12*FGAS(ASOC_UG1,  &
              & :)
    rct(82,:) = 4.0e-12*FGAS(ASOC_UG10,  &
              & :)
    rct(83,:) = 4.0e-12*FGAS(ASOC_UG1e2,  &
              & :)
    rct(84,:) = 4.0e-12*FGAS(ASOC_UG1e3,  &
              & :)
    rct(85,:) = 4.0e-12*FGAS(NON_C_ASOA_UG1,  &
              & :)
    rct(86,:) = 4.0e-12*FGAS(NON_C_ASOA_UG10,  &
              & :)
    rct(87,:) = 4.0e-12*FGAS(NON_C_ASOA_UG1e2,  &
              & :)
    rct(88,:) = 4.0e-12*FGAS(NON_C_ASOA_UG1e3,  &
              & :)
    rct(89,:) = 4.0e-12*FGAS(BSOC_UG1,  &
              & :)
    rct(90,:) = 4.0e-12*FGAS(BSOC_UG10,  &
              & :)
    rct(91,:) = 4.0e-12*FGAS(BSOC_UG1e2,  &
              & :)
    rct(92,:) = 4.0e-12*FGAS(BSOC_UG1e3,  &
              & :)
    rct(93,:) = 4.0e-12*FGAS(NON_C_BSOA_UG1,  &
              & :)
    rct(94,:) = 4.0e-12*FGAS(NON_C_BSOA_UG10,  &
              & :)
    rct(95,:) = 4.0e-12*FGAS(NON_C_BSOA_UG1e2,  &
              & :)
    rct(96,:) = 4.0e-12*FGAS(NON_C_BSOA_UG1e3,  &
              & :)
