...
contains
  ...
  do isrc = 1, Nsources
     ...
     if(iem<1)then
        ...
        if(ix<0)then
           ix=find_index(lf_src(isrc)%species ,species(:)%name, any_case=.true.)
     ...
     if(MasterProc)then
        write(*,*)'lf pollutant : ',lf_src(isrc)%species
...
!stop
end subroutine out_lf

subroutine av_lf(dt,End_of_Day)
