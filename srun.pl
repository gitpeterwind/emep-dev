...
# Snowstorm. The main advantage of this script is that
...
# some data is copied on all nodes (it is read by all nodes )
	...
	close(DATACP);
	   print "The model will run on nodes:  \n";
	system("cat $ENV{PBS_NODEFILE}   ");#write out which nodes are used
...
#     system("   cp ./*day.nc $RESDIR");
#     system("   cp ./*hour.nc $RESDIR");
#     system("cp ./${runlabel1}_RunLog $RESDIR");
    
    
#clean up work directories and links
    open (DATACP, "|mpiexec -comm none -pernode rm -rf $tempworkdir ") || 
	die "Unable to rm data. Exiting.\\n" ;
