...
!
!   Initialize and finish the modules:
!     \bv
...
!  \bv
!    #define IF_NOT_OK_RETURN(action) if (status/=0) then; write (*,'("ERROR in prog")'); action; return; end if
!
!    call aaa( status )
!    IF_NOT_OK_RETURN(status=1)
!  \ev
!  Combined with the 'GO_Print' features, the macro's could become:
!  \bv
...
contains
  ...
  subroutine GO_Init( status )
    ...
    use GO_Timer, only : GO_Timer_Init
    use GO_Par  , only : GO_Par_Init
  ...
  subroutine GO_Done( status )
    ...
    use GO_Par  , only : GO_Par_Done
    use GO_Timer, only : GO_Timer_Done
    ...
    ! done with parallel tools:
    call GO_Par_Done( status )
    IF_NOT_OK_RETURN(status=1)
    
    ! done with timing module:
