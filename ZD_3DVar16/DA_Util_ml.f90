#define TRACEBACK \
  write(gol,'("in ",a," (",a,", line ",i5,")")')rname, __FILE__, __LINE__;call goErr()
#define ERROR(action) \
  action;call goErr();TRACEBACK;status=1;return
#define IF_NOT_OK_ERROR(ok,action) \
  if(status/=ok)then;ERROR(action);endif
!#define USE_GO
...
private 
public :: norm
public :: GO_Init, GO_Done, GO_Par_Setup, T_Domains
...
! --- begin ----------------------------------
  ...
  ! ok
  status = 0
endsubroutine Domains_InitLocal
...
endsubroutine Domains_Intersection
...
!write (gol,*) '   rdispls      : ', rdispls; call goPr
  ...
  ! ok
  status = 0
endsubroutine Domains_Swap_2d_r8
...
! --- begin -----------------------------------
  ...
  ! ok
  status = 0
endsubroutine Domains_Swap_3d_r8
...
! --- begin -----------------------------------
  ...
  ! ok
  status = 0
endsubroutine Domains_Swap_4d_r8
#endif
!+------------------------------------------------------------------
!
!+------------------------------------------------------------------
