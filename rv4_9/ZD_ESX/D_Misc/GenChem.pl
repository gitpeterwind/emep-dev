...
#
 our $DEBUG = 0;                  # Set >0 for extra output. 2 gives loads!
 #MAY6 my $DIMFLAG  =  "k";       # could be ix,iy,iz for 3-D models if wanted
 my $DIMFLAG  =  "k";       # could be ix,iy,iz for 3-D models if wanted
# my $ESXFLAG  =  ",nzmid";  # ESX special
 my $ESXFLAG  =  ",k";  # ESX special
...
#      print *, \"CHEMDBG SPEC \", P, L, xnew(SPEC)\n" ; 

 $CHEMEQN_TEXT .=  "print *, \"CHEMDBG SPEC \", P, L, xnew(SPEC)" if $DEBUG>1 ; 
...
#  but for simplicity we define it anyway.... (ESX check)
 ...
 #ESX  use Met1D, m=> amk
 #ESX use ModelConstants,     only : KMAX_MID,KCHEMTOP,DebugCell,DEBUG_RUNCHEM"
...
$rcttext[$nrct] = "rct($nrct,:)";    # older ,$DIMFLAG)";
		#MAY6 $rcttext[$nrct] = "rct($nrct)";    # older ,$DIMFLAG)";
#		$rcttext[$nrct] = "rct($nrct,$DIMFLAG)";    # older ,$DIMFLAG)";
		$k = "rct($nrct$ESXFLAG)";
		#ESX $k = "rct($nrct,$DIMFLAG)";
...
#
#............... print_species ..........................
#   outputs fortran-format files GenOut_adv.inc
#   which look like, e.g.:
#
#    Genspec_adv.inc
#        NSPEC_ADV=66
...
########################################################################
    my ( $i, $top, $aero_txt );
#ESX    my ( %txt )       = ( $tot => "tot",  $adv => "adv", $shl => "shl" );
#ESX    my ( %nspec_txt ) = ( $tot => "NSPEC_TOT",
#ESX                          $adv => "NSPEC_ADV",
#ESX                          $shl => "NSPEC_SHL" );
    ...
    open(F,">CM_ChemSpecs.f90");
#ESX added:
        $module = "ChemSpecs";
        $Use = "none";
        start_module($module,\*F,$Use);
#ESX -----------
    printall( "ESXOUT TESTING \n");
    print F "!ESXOUT TESTING \n";

    foreach my $s ( $tot, $adv, $shl ) {
	next if $nspecies[$s] == 0 ;
	printall("PROCESS $s TXT $txt{$s} NSPEC $nspecies[$s] \n");

	#ESX $module = "ChemSpecs_$txt{$s}";
        #ESX $Use = "none";
        #ESX start_module($module,\*F,$Use);
...
#ESX     $Use    ="
#ESX  use ChemSpecs_tot  ! => NSPEC_TOT, species indices
#ESX  use ChemSpecs_shl, only: NSPEC_SHL
#ESX  use ChemSpecs_adv, only: NSPEC_ADV";
#ESX
#ESX    print F <<"END_CHEMSTART";
...
#
#     my $module = "ChemSpecs_maps";
#     my $Use  = "
#       use ChemSpecs_adv, only : NSPEC_ADV
#       use ChemSpecs_shl, only : NSPEC_SHL
#     ";
#
#     open(F,">$module.inc");
#     start_module( $module, \*F, $Use);
...
#######################################################################
  ...
  #ESX my $module = "ChemRates_" . $rctype . "" ;
  my $module = "ChemRates" ;
  my $subargs  = "debug_level";                         # can be blank
  my $arglines = "integer, intent(in) :: debug_level\n" ; #  " " as subargs
  open(F,">CM_ChemRates.f90");
  start_module( $module, \*F, $UsedVariables{$rctype} );

print "ESXDIMS HERE S$adv $nspec_txt{$adv} NCHEMRATES $nrct\n";
#Older:ESX??  integer, parameter, public :: $Nrctype = $nrc   !! No. coefficients
#Older:ESX real, save, public, dimension($Nrctype,$Krange) :: $defrc
...
#ESX	if ( $nrctroe > 0 ) {
#ESX		print F  "     real, dimension(KCHEMTOP:KMAX_MID) :: log300divt, logtdiv300\n";
#ESX		print F  "!     real, dimension(KCHEMTOP:KMAX_MID) :: BranchingNO\n";
#ESX		print F  "!     real, dimension(KCHEMTOP:KMAX_MID) :: BranchingHO2\n";
#ESX		print F  "       log300divt(:) = log(300.0*tinv(:))\n";
#ESX		print F  "       logtdiv300(:) = log(temp(:)/300.0)\n";
#ESX		print F  "!       BranchingNO(:) = 1.0e-11*xn_2d(NO,:)/ &
#ESX                !( 1.0e-11*xn_2d(NO,:) + 4.2e-12*exp(180*TINV(:))*xn_2d(HO2,:) )\n";
#ESX		print F  "!       BranchingHO2(:) = 1.0 - BranchingNO(:)\n\n";
...
#MAY6			$tmpout =~ s/\,iq\)/\)/ ;    # Remove dimension flag here
...
###############################################################################
 sub print_emisstuff {
	...
	printall( "ENTERING EMIS print $Nemis\n");
        open(EMIS,">CM_Emis$nam.inc") or die "FAIL EMIS_$nam\n";
...
sub process_Jrate {
	my $rate = shift;
	$rate =~ /DJ\( (\w+) /x;  # Gets e.g. IDNO2 from DJ(IDNO2,K), Into $1
	my $id = $1;              #  
	#$rate =~ s/DJ/rcphot/;
	$rate = "rcphot($id$ESXFLAG)";  # Rename for EMEP/ESX
	my ( $found, $dj ) = (0,-1);
	foreach $dj ( @photol_rates ) {
		$found = 1 if $id eq $dj ;
		printall( "PHOTOL FOUND: $dj \n" ) if $found;
	}
	push( @photol_rates, $id ) unless $found;
	#die "RCPHOT ARG HERE: $rate TERM $id ";
	return $rate
}
#########################################################################
sub process_emis {
 ...
 $rate = "rcemis($spec,k)" unless $found ; # Only add if not already set
	        #MAY6 $rate = "rcemis($spec)" unless $found ; # Only add if not already set
