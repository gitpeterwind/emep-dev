...
!   real(4)
!   real(8)
! In some cases also character strings are supported,
! this probably requires an extra argument MAXLEN:
...
!
! Gather similar shaped data from all other process on root.
! The receive buffer on the root should have the size of 
! input data times number of processes.
! NOTE: Original MPI routine handes different send/recv types
! and therefore needs the sendcount/recvcount numbers;
! however, for single data type the following improvements could be made:
!  - sendcount should be derived from sendbuf
!  - recvcount is the same as send count
!
!   subroutine MPIF90_Gather( sendbuf, sendcount, &
!                             recvbuf, recfcount, &
!                             root, comm, status )
!     <dtype>, intent(in)           ::  sendbuf<shp>  ! size n
!     integer, intent(in)           ::  sendcount     ! n
!     <dtype>, intent(out)          ::  recvbuf<shp>  ! size n*nproc
!     integer, intent(in)           ::  recvcount     ! n
!     integer, intent(in)           ::  root
!     integer, intent(in)           ::  comm
!     integer, intent(out)          ::  status
!   end subroutine MPIF90_Gather
!
! Gather similar shaped data from all other process on each pe.
! The receive buffer should have the size of input data times 
! number of processes.
!
!   subroutine MPIF90_AllGather( sendbuf, recvbuf, comm, status )
!     <dtype>, intent(in)           ::  sendbuf<shp>  ! size n
!     <dtype>, intent(out)          ::  recvbuf<shp>  ! size n*nproc
!     integer, intent(in)           ::  comm
!     integer, intent(out)          ::  status
!   end subroutine MPIF90_AllGather
!
! Send varying number of elements from each process to root,
! and receive elements from all processes in return.
! NOTE: to be improved:
!  - gather sendcounts in recvcounts on root
!  - derive rdispls from recvcounts using MPIF90_Displacements
!
!   subroutine MPIF90_GatherV( sendbuf, sendcount , &
!                              recvbuf, recvcounts, rdispls, &
!                              comm, status )
!     <dtype>, intent(in)           ::  sendbuf<shp>   ! size n1 on pe1, n2 on pe2, ..
!     integer, intent(in)           ::  sendcount      ! n1 on pe1, n2 on pe2, ... ; 
!                                                      ! necessary, since could be zero!
!     <dtype>, intent(out)          ::  recvbuf<shp>   ! size sum_i(ni)
!     integer, intent(in)           ::  recvcounts(<nproc>)  ! (/n1,n2,.../)
!     integer, intent(in)           ::  rdispls(<nproc>)     ! (/0,n1,n1+n2,.../)
!     integer, intent(in)           ::  comm
!     integer, intent(out)          ::  status
!   end subroutine MPIF90_GatherV
!
