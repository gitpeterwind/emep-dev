...
#define IF_NF90_NOT_OK_STOP if (status/=NF90_NOERR) then; TRACEBACK; call CheckStop( .true., "STOP" ); end if
#endif
...
#ifdef with_ajs
  use GO               , only : goMem
  character(len=*), parameter  ::  rname = mname//'/readxn'
  integer                      ::  status
#endif
...
#ifdef with_ajs
      ! trap if iniconc should be skipped on request ...
      if ( trim(filename_read_3D) == 'None' ) then
        ! info ...
        if (MasterProc) write (*,'("Nest_ml/ReadXN: undefined IC file, continue ..")')
      else
        ! error ...
        call CheckStop( .true., "Nest_ml/ReadXN: IC file expected, use 'None' if should be skipped" )
      end if
#endif
    end if
...
#ifdef with_ajs
    ! testing ..
    call goMem( rname//' - MEMORY rrr1', status )
    IF_NOT_OK_STOP
#endif
...
#ifdef with_ajs
    ! testing ..
    call goMem( rname//' - MEMORY rrr2', status )
    IF_NOT_OK_STOP
#endif
  end if
...
subroutine read_newdata_LATERAL(ndays_indate)
  ...
  use GO               , only : goMem
  character(len=*), parameter  ::  rname = mname//'/read_newdata_LATERAL'
  integer                      ::  status2
#endif
...
#ifdef with_ajs
    ! testing ..
    call goMem( rname//' - MEMORY sss1  ', status )
    IF_NOT_OK_STOP
#endif
...
#ifdef with_ajs
    ! testing ..
    call goMem( rname//' - MEMORY sss2a ', status )
    IF_NOT_OK_STOP
#endif
...
#ifdef with_ajs
    ! testing ..
    call goMem( rname//' - MEMORY sss3  ', status )
    IF_NOT_OK_STOP
#endif
...
#ifdef with_ajs
    ! testing ..
    call goMem( rname//' - MEMORY sss4  ', status )
    IF_NOT_OK_STOP
#endif
...
#ifdef with_ajs
    ! testing ..
    call goMem( rname//' - MEMORY sss5  ', status )
    IF_NOT_OK_STOP
#endif
...
#ifdef with_ajs
    call goMem( rname//' - MEMORY sss5a ', status )
    IF_NOT_OK_STOP
#endif
    n=adv_bc(bc)%ixadv
    if(MasterProc)then
#ifdef with_ajs
      call goMem( rname//' - MEMORY sss5b ', status )
      IF_NOT_OK_STOP
#endif
      if(DEBUG_NEST.or.DEBUG_ICBC) write(*,"(2(A,1X),I0,'-->',I0)")&
        'Nest: DO_BC',trim(adv_bc(bc)%varname),bc,n
#ifdef with_ajs
      call goMem( rname//' - MEMORY sss5c ', status )
      IF_NOT_OK_STOP
      write (gol,'(a," - MEMORY read ",a)') rname, trim(adv_bc(bc)%varname); call goPr
#endif
      ! Could fetch one level at a time if sizes becomes too big
      call check(nf90_inq_varid(ncFileID,trim(adv_bc(bc)%varname),varID))
#ifdef with_ajs
      call goMem( rname//' - MEMORY sss5d ', status )
      IF_NOT_OK_STOP
#endif

#ifdef with_ajs
      status = nf90_get_var( ncFileID, varID, data, &
                               start=(/ 1,1,1,itime /), &
                               count=(/ GIMAX_ext,GJMAX_ext,KMAX_ext_BC,1 /) )
      IF_NF90_NOT_OK_STOP
      call goMem( rname//' - MEMORY sss5d0', status2 )
#else
      call check(nf90_get_var(ncFileID, varID, data &
            ,start=(/ 1,1,1,itime /),count=(/ GIMAX_ext,GJMAX_ext,KMAX_ext_BC,1 /) ))
      call goMem( rname//' - MEMORY sss5d1', status2 )
#endif
      status = nf90_get_att(ncFileID,VarID,"scale_factor",scale_factor)
#ifdef with_ajs
      call goMem( rname//' - MEMORY sss5d2', status2 )
#endif
      if(status==nf90_noerr) data=data*scale_factor
#ifdef with_ajs
      call goMem( rname//' - MEMORY sss5d3', status2 )
#endif
      status = nf90_get_att(ncFileID,VarID,"add_offset",add_offset)
#ifdef with_ajs
      call goMem( rname//' - MEMORY sss5d4', status2 )
#endif
      if(status==nf90_noerr) data=data+add_offset
#ifdef with_ajs
      call goMem( rname//' - MEMORY sss5e ', status )
      IF_NOT_OK_STOP
#endif
...
#ifdef with_ajs
      call goMem( rname//' - MEMORY sss5f ', status )
      IF_NOT_OK_STOP
#endif
      if(unitscale/=1.0) data=data*unitscale
    end if
#ifdef with_ajs
      call goMem( rname//' - MEMORY sss5g ', status )
      IF_NOT_OK_STOP
#endif
...
#ifdef with_ajs
      call goMem( rname//' - MEMORY sss5h ', status )
      IF_NOT_OK_STOP
#endif
...
#ifdef with_ajs
      call goMem( rname//' - MEMORY sss5i ', status )
      IF_NOT_OK_STOP
#endif
  end do DO_BC
...
#ifdef with_ajs
    ! testing ..
    call goMem( rname//' - MEMORY sss6  ', status )
    IF_NOT_OK_STOP
#endif
...
#ifdef with_ajs
    ! testing ..
    call goMem( rname//' - MEMORY sss2b ', status )
    IF_NOT_OK_STOP
#endif
