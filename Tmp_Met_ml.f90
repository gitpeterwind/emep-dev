...
! hb new meteoread
  ...
  use NH3variables_ml,  only: TSPERDAY,MAXTIMESTEPS,NNH3,I_ISO_STABLE,&
  ...
  use Io_ml,            only: open_file, ios, IO_NH3 
  ...
  use PhysicalConstants_ml, only: T0
  implicit none
  private
...
subroutine daydagt(T2,DAYDAGTEMP)
  ...
  !******************************************************************
  use NH3variables_ml, only   : TSPERDAY
  use TimeDate_ml, only       : current_date,day_of_year
  real, INTENT(IN) :: T2(:)
...
end module calc_emis_potential_ml
