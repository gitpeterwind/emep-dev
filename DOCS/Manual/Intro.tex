...
\QUERY{Waiting for Garry Hayman and Mike Jenkin to confirm co-authorship}
...
\NEW{(The model can be run for shorter periods
than one year though, so users only need to download meteorology for the
...
have a horizontal resolution of 50$\times$50 km$^2$ \NEW{(at 60$^\circ$N)}
and are defined on a
polar stereographic projection with 20 sigma levels vertically. 
The model \NEW{is very flexible with regard to the horizontal
resolution, in that it readily makes use of}
\DEL{uses the same horizontal and vertical grid resolution as the}
