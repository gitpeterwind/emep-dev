...
datasets. \NEW{The light and temperature depencies are similar to those
...
This file, based on \DEL{HIRLAM} \NEW{the meteorological model's} roughness length, is used
...
is applied to all countries by default. 

The specials files are in general optional, and can be used to specify
speciation for particular countries or SNAP sectors. Th
1st column specifies the country code of interest, the second the SNAP sector. 
...
\NEW{(PFT) gives a plant-functional type code (for future use),
the fifth gives}
...
LAI minimum  and maximum ($m^{2}/m^{2}$) and \DEL{finally}
columns 14 and 15
