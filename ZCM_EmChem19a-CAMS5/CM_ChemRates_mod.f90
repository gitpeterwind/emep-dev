...
module ChemRates_mod
  ...
  implicit none
  private
  character(len=*),parameter, public :: CM_schemes_ChemRates = " EmChem19a PM_ECf PM_POM25 Aqueous_EmChem16x Aero2017nx Ash ShipNOx PM_FFireInert SeaSalt DustExtended BVOC_SQT_NV BVOC_IsoMT1_emis"
...
contains
  ...
  subroutine setChemRates()
    ...
    rct(80,:) = EC_AGEING_RATE()
    rct(81,:) = HYDROLYSISN2O5()
    rct(82,:) = UPTAKERATE(CNO3,  &
              & GAM=0.001,  &
              & S=S_M2M3(AERO%PM,  &
              & :))
    rct(83,:) = UPTAKERATE(CHNO3,  &
              & GAM=0.1,  &
              & S=S_M2M3(AERO%DU_C,  &
              & :))
    rct(84,:) = UPTAKERATE(CHNO3,  &
              & GAM=0.01,  &
              & S=S_M2M3(AERO%SS_C,  &
              & :))
    rct(85,:) = UPTAKERATE(CHO2,  &
              & GAM=0.2,  &
              & S=S_M2M3(AERO%PM,  &
              & :))
