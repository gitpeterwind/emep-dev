...
!      call printCDF('LU', landuse_in(:,:,1),'??')
  ...
  subroutine ReadLanduse_CDF(filefound)
    ...
    !OLD:So far only basic version for use in TNO7. Under construction
    !
    logical :: filefound
    integer :: i,j,lu, ilu, index_lu, maxlufound, iam, iveg, ifile, nFiles 
    real :: sumfrac
    integer, save :: ncalls=0

    character(len=40) :: varname, fShort
    character(len=200) :: fname1,fname2, fName, msg
!XX    character(len=200), dimension(2) :: fnamesLC
    ...
    integer, dimension(LIMAX,LJMAX):: landuse_ncodes ! tmp, with all data
    integer, dimension(LIMAX,LJMAX,NLUMAX):: landuse_codes ! tmp, with all data
!    integer, dimension(LIMAX,LJMAX):: landuse_n = 0  ! tmp, with all data
    ...
    FILELOOP: do ifile = 1, nFiles ! size(LandCoverInputs%MapFile(:))
      ...
      if ( MasterProc .and. fexist) then
       ...
       !are fexist and status equiv.?:
       !write(*,*) dtxt//"LANDUSE: ?? ", ifile,fexist, status, nf90_noerr
      end if

      if(status /= nf90_noerr) then
         if ( MasterProc )write(*,'(a,i2,1x,a)') dtxt//"LANDUSE: NOT found ", &
                  ifile,trim(fShort)
         cycle !CLM
      end if
      if ( ifile==1 ) EuroFileFound = .true. ! CRUDE; DANGEROUS; ARGH!
      if ( ifile==2 ) GlobFileFound = .true. ! CRUDE; DANGEROUS; ARGH!
!      if ( dbg )write(*,'(a,i2,1x,a)') dtxt//"LANDUSE: FOUND ", &
!                  ifile,trim(fShort)
     ...
     !get list of variables
      ...
      VARIDLOOP1: do varid=1,nVariables ! all variables in file
          ...
          if ( mydbg ) write(*,*) trim(msg)//": Used from ", trim(varname), ' ', ewords(3)
     ...
     !       print *, dtxt//"Does ", me, ifile, trim(varname), trim(ewords(2))
          ...
          if( ewords(2) == "IAM_VEG" ) cycle  ! No IAM veg to process
          !=========================
         ...
         !CHECK HERE to see if we already have this landcode:...
          ...
          !if(mydbg) print *, dtxt//'LUVARNAME', ifile, ilu, trim(varname)
          if (  lu > 0 ) then
...
!DS                  needed=.false.,debug_flag=.false.)
      ...
      ! MERGE Euro and Glob
       ...
       if(MasterProc) then
            write(*,*)"LandDefs DONE ", ilu, Land_codes(ilu), maxval( landuse_in ), minval(landuse_in)
  ...
  ! We divide the area  by the nFluxVegs to keep the total area small
