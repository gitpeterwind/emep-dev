...
my @MAKE = ("gmake",  "-j2", "--makefile=Makefile_snow");  # CHANGE Makefile as appropriate
...
my $MYNAME      = "mifads"; #-- CHANGE as appropriate!!!, e.g. = "fred";
my $USER        =  $MYNAME;

my $HOMEROOT     = "/home";      
my $WORKROOT     = "/global/work";      

# DataDir    = Main general Data directory ---   CHANGE as appropriate!!!

our $DataDir      = "/global/work/mifads/input_data";
...
my $MetDir        = "$DataDir/EMEP_metdata/$year" ;
my $EMIS_INP      = "$GRID_DATA/Emissions";     # Emissions top dir.
my $emisdir       = "$EMIS_INP/2007-Trend${year}-V9";
...
# Emissions setup:
    ...
    $ifile{"$COMMON/pm25split.defaults.$Split"} = "pm25split.defaults";
...
# Seasonal stuff  ----    Can't we improve this? e.g. every month?
    ...
    $ifile{"$GRID_DATA/rough.170"} = "rough.170"; # Roughness length;
