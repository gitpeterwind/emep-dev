program emep_Main
  ...
  use EMEP_CSO_mod,      only: NTIMING_CSO, EMEP_CSO_Init, EMEP_CSO_Done
...
!TEST
  ...
  ! init simulation of satellite retrievals;
  ! requires just defined subdomain partitioning
  call EMEP_CSO_Init( status )
  call CheckStop( status, dtxt//"EMEP_CSO_Init in emepctm" )
...
!TEST  call define_chemicals()    ! sets up species details
  ...
  ! done with simulation of satellite retrievals:
  call EMEP_CSO_Done( status )
  call CheckStop( status, dtxt//"EMEP_CSO_Done in emepctm" )
