...
!WN17, Gets PAR values, W/m2 here
  ...
  !   only put-out if this is NOT a run with analysis enabled:
  call EMEP_CSO_Simulate( 'fg', .not. ANALYSIS, status )
  call CheckStop( status, "EMEP_CSO_Simulate in PhyChem_mod/PhyChe" )
  ! end timing:
  ...
  if(ANALYSIS)then
    ...
    !  "analysis" selection;
    !   put out now:
    call EMEP_CSO_Simulate( 'an', .true., status )
    call CheckStop( status, "EMEP_CSO_Simulate in PhyChem_mod/PhyChe" )
    ! end timing:
    call Add_2timing( TIMING_CSO, tim_after, tim_before )
  end if
