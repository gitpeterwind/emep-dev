...
!____________________________________________________________________________
  ...
  integer, public, parameter ::  NGLOB_BC  = 20 ! Dust 18 ! No. species setup here
...
!  SpecBC(IBC_HNO3 ) = sineconc( 0.1  , 15.0, 0.03, 999.9, 0.05, 0.05,PPB )!M, bcKz vmin
 ...
 !Dust
 ! the factor PPB converts from PPB to mixing ratio.
...
! grid coordinates of Mace Head
          ...
          case ( IBC_SO2, IBC_SO4, IBC_HCHO, IBC_CH3CHO, IBC_NH4_f, IBC_NO3_f  )
...
!//__ Dust (Sahara BIC from monthly 2000 data from CTM)
          case ( IBC_DUST_c, IBC_dust_f )
            if (DO_SAHARA) then
              print *, 'going set bc_rawdata for DUST; used = ', used
              ! set zero
              bc_rawdata(:,:,:) =    0.0

              ! set file name, either coarse or fine
              if (ibc == IBC_DUST_c) then
                 write(unit=fname,fmt="(a11,i2.2)") "DUST_c_ext.",month
              elseif (ibc == IBC_DUST_f) then
                 write(unit=fname,fmt="(a11,i2.2)") "DUST_f_ext.",month
              endif
