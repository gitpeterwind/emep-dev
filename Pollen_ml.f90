...
!  use SubMet_ml,            only : Sub, Grid !%t2 %rh
  ...
  use Par_ml,               only : limax, ljmax, MAXLIMAX,MAXLJMAX   ! => x, y dimensions
...
!-------------------------------------
  ...
  integer, public, save,allocatable, dimension(:,:):: p_day
  ...
  real,private, parameter  :: &
   ...
   logical, private  :: pollen_first = .true. 
...
!<------------------------------------------------------------------------!
   ...
   integer , save      :: ipoll,inat_POLL,itot_POLL   !DS replaces Pollen_b
   real                :: scale,lim,invdz,n_to_mPOLL!, moleccm3s_2_kgm2h
...
! Reas in the different fields
   if (.not. my_first_call_pollen ) then
       pollen_first  = .false.
   ...
   if ( my_first_call_pollen ) then 
    ...
    itot_POLL = find_index( "POLLEN_B", species(:)%name    )
      ...
      allocate(pollen_left(MAXLIMAX,MAXLJMAX))!,heat_rel(MAXLIMAX,MAXLJMAX))
      ...
      allocate(p_day(MAXLIMAX,MAXLJMAX))
...
! calculating grains to molecular weight   grains/m2*s --> mol/cm3*s 
    ...
    !   ipoll = find_index("POLLEN_B", species(:)%name ) 
       n2m = n_to_mPOLL * invdz * AVOG / species(itot_POLL)%molwt * 1.0e-18
...
if (birch_frac(i,j)==0.000 .or. h_c(i,j)==0.000) then
      scale = 0.00
else 
...
!if (h(i,j) .gt. 200) write(*,*)'POLLERROR :',h(i,j),heatsum(i,j),t2_nwp(i,j,1)

     heatsum(i,j) = h(i,j) ! if you want to output the heatsum field
 !    write(*,*)'HEAT_REL',heat_rel(i,j)!'POLLEN heatsum',heatsum(i,j),'i,j',i,j
      ...
      if (heatsum(i,j)<lim .or.t2_nwp(i,j,1) <T_cutoff &
                     .or. surface_precip(i,j)>prec_max &
                     .or. R(i,j)>N_TOT*corr(i,j) &
                     .or. pollen_first) then 
         scale = 0
...
!  Scale is calculated from the different meteorological conditions using functions below  
         scale= corr(i,j)*(t2_nwp(i,j,1) - T_cutoff)/(dH*24*60*60)* &
       ...
       ! Pollen_prod(QPOL,i,j) =N_tot*scale*birch_frac(i,j) !The pollen grains production
        rcpoll = N_TOT*scale*birch_frac(i,j) !The pollen grains production
        AreaPOLL(i,j) = N_TOT*scale*birch_frac(i,j)*60*60 ! [grains/m2/h]
!<-----------------------------------------------------------------------------------------------
  !!! Need to convert grains/m2*s --> mol/cm3*s
        Pollen_rest(i,j) = N_TOT - R(i,j) ! pollen grains left pr m2 after release
        rcpoll = rcpoll * n2m
        pollen_left(i,j) = Pollen_rest(i,j)/N_TOT
        
         
   
    if (debug_flag .or. DEBUG_POLLEN ) write(6,'(a,2f7.4,f6.2)') "Result: ", &
             rcpoll, R(i,j)/N_TOT,AreaPOLL(i,j)
