...
c-----------------------------------------------------------------------
c
      use mpi, only : MPI_COMM_WORLD, MPI_SUM, MPI_INTEGER, MPI_SUCCESS !,MPI_ALLREDUCE
c
      implicit none
c
c     arguments
...
c     if (epsg.le.0.d0 .or. epsg.gt.1.d0) then
     ...
     &        impres
      ...
      call MPI_ALLREDUCE( n_loc, n_sum, 1, MPI_INTEGER,
     &                 MPI_SUM, MPI_COMM_WORLD, status )
...
c
      ...
      call MPI_ALLREDUCE( ntravu, n_sum, 1, MPI_INTEGER,
     &                 MPI_SUM, MPI_COMM_WORLD, status )
      if(status/=MPI_SUCCESS) then
        write(io,*) 'ERROR in ', __FILE__, ' line ', __LINE__
        reverse=-1; return
      endif
      if(impres.ge.1)write(io,931) ndz_loc,ndz_glb,ntravu,n_sum,m
  931 format (/5x,"allocated memory (ndz) :",i9,",",i9,
     &        /5x,"used memory :           ",i9,",",i9,
...
c----
c 
      use mpi, only : MPI_COMM_WORLD, MPI_SUM, MPI_MAX,
     &   MPI_DOUBLE_PRECISION, MPI_SUCCESS !,MPI_ALLREDUCE
c
      implicit none
c
c         arguments
...
c
      ...
      ! global maximum:
      call MPI_ALLREDUCE( tmin_loc, tmin, 1, MPI_DOUBLE_PRECISION,
     &                 MPI_MAX, MPI_COMM_WORLD, status )
      if(status/=MPI_SUCCESS) then
        write(io,*) 'ERROR in ', __FILE__, ' line ', __LINE__
        reentry=-1; return
      endif
...
c
          ...
          ! global sum:
          call MPI_ALLREDUCE( ps_loc, ps, 1, MPI_DOUBLE_PRECISION,
     &                 MPI_SUM, MPI_COMM_WORLD, status )
          if(status/=MPI_SUCCESS) then
            write(io,*) 'ERROR in ', __FILE__, ' line ', __LINE__
            reentry=-1; return
...
c
          ...
          ! global sum:
          call MPI_ALLREDUCE( ps2_loc, ps2, 1, MPI_DOUBLE_PRECISION,
     &                 MPI_SUM, MPI_COMM_WORLD, status )
          if(status/=MPI_SUCCESS) then
            write(io,*) 'ERROR in ', __FILE__, ' line ', __LINE__
            reentry=-1; return
...
c ----
c
      use mpi, only : MPI_COMM_WORLD, MPI_LOR, MPI_LOGICAL, MPI_SUCCESS !,MPI_ALLREDUCE
c
      implicit none
c
c --- arguments
...
c
      ...
      call MPI_ALLREDUCE( changed, any_changed, 1, MPI_LOGICAL,
     &                 MPI_LOR, MPI_COMM_WORLD, status )
      if(status/=MPI_SUCCESS) then
        write(io,*) 'ERROR in ', __FILE__, ' line ', __LINE__
        reentry=-1; return
      endif
...
c--------0---------0---------0---------0---------0---------0---------0--
c
      subroutine euclid_weight_mpi(n, x, y, ps, izs, rzs, dzs)
c
      use mpi, only : MPI_COMM_WORLD, MPI_IN_PLACE, MPI_SUM,
     &                MPI_DOUBLE_PRECISION, MPI_SUCCESS !,MPI_ALLREDUCE
      implicit none
      ! --- in/out ---------------------------------
      integer, intent(in)   :: n, izs(*)
      real(8), intent(in)   :: x(n), y(n)
      real(8), intent(out)  :: ps
      real(4), intent(in)   :: rzs(*)
      real(8), intent(in)   :: dzs(*)
      ! --- const ----------------------------
      character(len=*), parameter :: rname = 'euclid_mpi'   
      ! --- local ----------------------------------    
      integer               :: i, status
      ! --- begin ----------------------------------
    
      ps = 0.d0         ! init local sum
      do i = 1, n       ! add elements
        ps = ps + x(i) * y(i) * izs(i)
      enddo
    
      ! collect global sum:
      call MPI_ALLREDUCE(MPI_IN_PLACE, ps, 1, MPI_DOUBLE_PRECISION,
     &                 MPI_SUM, MPI_COMM_WORLD, status)
      if(status/=MPI_SUCCESS) then
        write (*,'("ERROR in ",a," (",a,", line ",i5,")")')
     &    rname,__FILE__,__LINE__
        flush(6)
        call MPI_ABORT(MPI_COMM_WORLD,9,status)
      endif
      endsubroutine euclid_weight_mpi
c
c--------0---------0---------0---------0---------0---------0---------0--
c
